...
server <- function(input, output, session) {
  ...
  output$table_qa_metrics <- renderDT({
    n_low_critics <- if ("low_critic_count" %in% names(df)) sum(df$low_critic_count, na.rm = TRUE) else NA
    n_genres      <- if ("primary_genre" %in% names(df)) n_distinct(df$primary_genre[!is.na(df$primary_genre)]) else NA
